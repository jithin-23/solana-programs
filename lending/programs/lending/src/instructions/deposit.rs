...
pub fn process_deposit(ctx: Context<Deposit>, amount: u64) -> Result<()> {
    ...
    let cpi_context = CpiContext::new(ctx.accounts.token_program.to_account_info(), transfer_api_accounts);
    let decimals = ctx.accounts.mint.decimals;
    transfer_checked(cpi_context, amount, decimals)?;

    let bank = &mut ctx.accounts.bank;

    if bank.total_deposists == 0 {
        bank.total_deposists = amount;
        bank.total_deposit_shares = amount;
    }
    let deposit_ratio = amount.checked_div(bank.total_deposists).unwrap();
